...
class SaleController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        foreach ($product_id as $i => $product_id_value) {
            if(isset($net_unit_price[$i])) {
                $validation = $this->validateLowestPrice($product_id_value, $net_unit_price[$i]);
                if(!$validation['valid']) {
    ...
    public function validateLowestPrice($product_id, $selling_price)
    {
        $product = Product::select('id', 'name', 'lowest_price')->find($product_id);
        
        if($product && $product->lowest_price && $selling_price < $product->lowest_price) {
            return [
                'valid' => false,
                'message' => trans('file.Price below minimum not allowed') . '. ' . 
                           trans('file.The selling price cannot be lower than the minimum price set for this product') . 
                           ' (' . $product->name . ': ' . trans('file.Lowest Price') . ' = ' . number_format($product->lowest_price, 2) . ')'
            ];
        }
    ...
    public function validateSalePrice(Request $request)
    {
        $product_ids = $request->input('product_id');
        $net_unit_prices = $request->input('net_unit_price');
        
        $errors = [];
        
        if($product_ids && $net_unit_prices) {
            foreach($product_ids as $index => $product_id) {
                if(isset($net_unit_prices[$index])) {
                    $validation = $this->validateLowestPrice($product_id, $net_unit_prices[$index]);
                    if(!$validation['valid']) {
    ...
    public function validateSinglePrice(Request $request)
    {
        $product_id = $request->input('product_id');
        $net_unit_price = $request->input('net_unit_price');
        
        if($product_id && $net_unit_price) {
            $validation = $this->validateLowestPrice($product_id, $net_unit_price);
            if(!$validation['valid']) {
                return response()->json([
                    'error' => true,
